...
class PDF extends FPDF
{
    function Header(){
        ...
        $this->Cell(0,10,iconv( 'UTF-8','TIS-620','ข้อมูลสวนผลไม้'),0,1,"C");
    }
    ...
    function headerTable(){
        ...
        $this->Cell(45,10,iconv( 'UTF-8','TIS-620','ชื่อสวน'),1,0,'C');
        ...
        $this->Cell(30,10,iconv( 'UTF-8','TIS-620','ตำบล'),1,0,'C');
        $this->Cell(30,10,iconv( 'UTF-8','TIS-620','อำเภอ'),1,0,'C');
...
//        $month=$_POST["month"];
        $this->AddFont('angsa','','angsa.php');
        $this->SetFont('angsa','',15);
        $stmt = $db->query($_POST["sql"]);
        $num=1;
        while($data = $stmt->fetch(PDO::FETCH_OBJ)){
            $this->Cell(15,10,$num,1,0,'C');
            $this->Cell(45,10,iconv( 'UTF-8','TIS-620',$data->pd_prefix.$data->pd_name."   ".$data->pd_lastname),1,0,'L');
            $this->Cell(45,10,iconv( 'UTF-8','TIS-620',$data->gd_name),1,0,'L');
            ...
            $this->Cell(30,10,iconv( 'UTF-8','TIS-620',$data->pd_district),1,0,'L');
            $this->Cell(30,10,iconv( 'UTF-8','TIS-620',$data->pd_city),1,0,'L');
...
$pdf->viewTable($db);
//    $pdf->Footer();
$pdf->Output("garden.pdf","F");

header("Location: garden.pdf");
?>
